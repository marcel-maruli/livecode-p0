...
function questLogger(quests, actions) {
  ...
  for (var i = 0; i < quests.length;i++){
    var hitung = 0
    var objHasil = {};
    for (var k = 0; k < quests[i]["requirements"].length;k++){
        for (var j = 0; j < actions.length;j++){
        // console.log (actions[j])
        if (actions[j]==quests[i]["requirements"][k]){
          hitung++;
      }
    }
  }
  // console.log(hitung)
  if (hitung==quests[i]["requirements"].length){
    objHasil = {quests: quests[i]["name"], status: "completed"}
    arrayHasil.push(objHasil)
    objHasil ={}
  }else if (hitung!==quests[i]["requirements"].length && hitung!==0){
    objHasil = {quests: quests[i]["name"], status: "on progress"}
    arrayHasil.push(objHasil)
    objHasil ={}
  }else if (hitung==0){
    objHasil = {quests: quests[i]["name"], status: "failed"}
    arrayHasil.push(objHasil)
    objHasil ={}
  }
}
