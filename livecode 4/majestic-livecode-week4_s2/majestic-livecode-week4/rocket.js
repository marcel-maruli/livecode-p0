...
function checkCoordinate (board) {
  ...
  for (var i = 1; i < board.length; i++){
    for (var j = 1; j < board[i].length; j++){
      ...
      var strX       = "";
      if (board[i][j]=="x"){
        strX += board[0][j] + board [i][0];
        penampungX.push(strX)
      }
      if (board[i][j]=='y'){
        strY = board[0][j] + board [i][0];
        penampungY.push(strY)
      }
      if (board[i][j]=='z'){
        strZ = board[0][j] + board [i][0];
        penampungZ.push(strZ)
      }
    }
  }
  var penampungHasil = [penampungX, penampungZ, penampungY]
  return penampungHasil
}
